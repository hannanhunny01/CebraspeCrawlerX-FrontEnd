...
import { isNumeric, isValidEmail, isValidNumber } from '../../Validators/validators';

/**
 * This file contains the Items component which displays the user's items and allows them to edit and register new contacts.
 * @file FILEPATH: /home/hannan/Desktop/CebraspeCrawlerX-FrontEnd/frontend/src/components/profile/messageCard/items.jsx
 */
function Items() {
  ...
  // disbale for  api sending buttons
  ...
  const registerProfile = async (item, index) => {           //api to initiate Code Sending  on desired profile
    ...
    if (item === "Whatsapp" && !isValidNumber(newContact[index])) {
      setInputErrorMessage("Numero Invalido");
      inputErrorFunc(index);

      return;
    }
   // console.log(item);
    if (item === "Email" && !isValidEmail(newContact[index])) {
      setInputErrorMessage("Email Invalido");
      inputErrorFunc(index);
      return;
    }
    setInputError(Array(3).fill(false));
    ...
    <div>
      {items.map((item, index) => (
        <div key={index}>
          ...
          {/*  zap and email check validation */}
          ...
          {/* zap and email part input error red color */}

          {inputError[index] && edit[index] &&
          ...
          {sendCode[index] && item.name !== "Telegram" &&
            ...
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-evenly" }}>
                ...
                <button className='send-code-button'  disabled={disableButton}  onClick={() => {
                  if(code[index].length === 6 && isNumeric(code[index])){
                  checkCode(item.name, index)}
                  else{
                    setOpenModal(true);
